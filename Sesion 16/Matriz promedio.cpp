...
public:
   ...
   void Suavizar(MatrizCuadradaReales original){
   		int suma, contador=0;
   	
   		for(int i = 0; i < util_fil_col; i++) 	// Rellenamos la suavizada
	        for(int j = i+1; j < util_fil_col; j++){
	        	suma=0;
	        	contador=0;
	        	for(int k=j; k<util_fil_col;k++){
	        		contador++;
	        		suma+=original.Elemento(i,k);
				}
				
				matriz_privada[i][j]=suma/contador;
				matriz_privada[j][i]=matriz_privada[i][j];
			}
	        	
	    for(int i = 0; i < 5; i++) 	// Rellenamos la diagonal de la suavizada
	        matriz_privada[i][i]=original.Elemento(i,i);
   }
